...
- The `real_app` fixture is not defined up-tree (owned by `hex-test-integration-setup`) → stop, the suite cannot collect without it. (No authenticated client is consumed here — Rule 8 — so the absence of the auth fixture set does not block this skill.)
