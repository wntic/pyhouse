...
1. **Pin PERSISTED state, not the in-memory entity.** Assert the write happened via the fake's `updated` call-record (`assert repo.updated == [id]`) AND read the value back (`(await repo.get_by_id(id)).status == DONE`). The fake returns a COPY and logs updates (`hex-test-application-handler`'s store-and-return-copies rule), so a body that mutates the entity but never calls `update()` **reds**. Asserting on the entity object the handler mutated in place pins nothing (it observes the in-memory mutation, not the persist).
...
3. **Exercise a non-boundary case, not only the boundary.** A limit test that only exercises one tier of a graded limit (or only the `>=` boundary) leaves the other tiers' limit-selection — and the "is the right limit even chosen?" logic — unpinned. Add a second tier and a value clearly inside the limit.
...
8. **Default-happy-path only.** No `fail_next_create=True` flags or `_should_raise` knobs. Tests needing one-off failures declare a private subclass at the **handler test module scope** (the pattern is owned by `hex-test-application-handler`):
...
- Spec asks to fake a handler's **concrete domain-service** dependency (e.g. `FooLimitPolicy`, injected as the class, not a Protocol) → stop, a structural fake won't type-check there; **subclass the service** (override the method under test, bypass `__init__`) or have the handler **inject via a Protocol**. This is the handler-test's call — see `hex-test-application-handler`'s rule that a concrete domain service cannot be faked structurally. Repository/capability fakes (this skill) are structural because their dependencies are Protocols.
