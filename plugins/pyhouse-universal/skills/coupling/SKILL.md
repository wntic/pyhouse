...
service-to-service strength must be the lowest: no service imports a sibling, and no service defines a
table or writes its own SQL (both rules in `flat-monorepo`, in the `pyhouse-flat` plugin). The schema
itself, though, is *shared model knowledge* that cannot be avoided — so the house puts it in one
owning package and serves it through repository methods (`flat-schema-package`), which turns every
